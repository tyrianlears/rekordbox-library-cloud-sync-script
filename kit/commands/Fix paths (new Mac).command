#!/bin/bash
# Fix paths after moving to a Mac with a different user name (dry run) — double-click to run. Same action as the button in "rekordbox Backup.app".
...
if [ -x "$KIT_BIN/rbk-fixpaths.sh" ]; then S="$KIT_BIN/rbk-fixpaths.sh"; else S="$HERE/rbk-fixpaths.sh"; fi
clear; echo "▶ Fix paths after moving to a Mac with a different user name (dry run)"; echo
/bin/bash "$S" ; RC=$?
