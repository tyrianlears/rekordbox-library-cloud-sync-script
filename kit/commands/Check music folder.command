#!/bin/bash
# Check music folder (is everything offline?) — double-click to run. Same action as the button in "rekordbox Backup.app".
...
if [ -x "$KIT_BIN/rbk-checkmusic.sh" ]; then S="$KIT_BIN/rbk-checkmusic.sh"; else S="$HERE/rbk-checkmusic.sh"; fi
clear; echo "▶ Check music folder (is everything offline?)"; echo
/bin/bash "$S" ; RC=$?
