...
- Design: `INSTALL_DIR` (scripts) and `APP_DIR` in `config.sh`; `RBK_LOCAL_BIN` = `~/Library/Application Support/rekordbox-backup/bin` always holds `rbk-launch.sh`, which launchd calls; it execs `$INSTALL_DIR/rbk-backup.sh` or, if missing, logs + notifies (12 h rate limit, state file `launch-miss-notified`) and exits 4. Config, state, lock, logs stay local. "Inside the backup folder" reuses the portable `kit/` copy that the installer already places there (no second copy); "Other folder…" creates `<folder>/rekordbox-backup-kit`. App: `~/Applications` or `/Applications` (falls back to `~/Applications` when not writable). Default stays local — Nick's choice ("local by default").
